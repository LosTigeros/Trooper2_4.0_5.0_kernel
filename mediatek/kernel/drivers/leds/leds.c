...
static int brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK0)
	{
        ...
        #ifdef __M8525__
            upmu_set_isink_ch0_step(0x02); // 12mA
        #else
            upmu_set_isink_ch0_step(0x0); // 4mA
        #endif
		...
		{
		#if 1 //defined(__M8510__)
            upmu_set_isink_ch0_en(0x0); // Turn off ISINK Channel 0
        #else
            upmu_set_isink_ch0_en(0x1); // Turn off ISINK Channel 0
        #endif
		}
		return 0;
	}
	else if(pmic_type == MT65XX_LED_PMIC_NLED_ISINK1)
	{
        ...
        #if defined(__M8512__)|| defined(__M8513__)
            upmu_set_isink_ch1_step(0x5); // 24mA
        #else
            upmu_set_isink_ch1_step(0x0); // 4mA
        #endif    
...
#elif defined CONFIG_ARCH_MT6572
#if defined(__M8510__) || defined(__M8512__) || defined(__M8526__)|| defined(__M8513__)
//for HALL
unsigned int bak_BL_status = 0;
EXPORT_SYMBOL(bak_BL_status);

void flashlight_isink_set_pmic(u32 level)
{
    brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK1,level,0); 
    brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK3,level,0); 
}
#endif
static int mt65xx_led_set_cust(struct cust_mt65xx_led *cust, int level)
{
	...
	int tmp_level = level;
#ifdef __M8525__
    static bool s_bBacklightOff = true;
#endif
//Mark out since the level is already cliped before sending in
/*
	if (level > LED_FULL)
		...
		case MT65XX_LED_MODE_PWM:
			if(strcmp(cust->name, "lcd-backlight") == 0)
			{
				...
				if(level == 0)
				{
					mt_pwm_disable(cust->data, cust->config_data.pmic_pad);
                #if defined(__I120__)
					brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,0,0);       //add  button light
				#endif
				
                #ifdef __M8525__
                    s_bBacklightOff = true;
                #endif						
				}
				else
				{
                #ifdef __M8525__
                    //每次开背光的时候，延时防止部分LCD出现闪屏现象
                    if (s_bBacklightOff == true)
                        msleep(150);
                    s_bBacklightOff = false;
                #endif
                ...
                #if defined(__I120__)
					brightness_set_pmic(MT65XX_LED_PMIC_NLED_ISINK0,255,0);    //add  button light
				#endif
