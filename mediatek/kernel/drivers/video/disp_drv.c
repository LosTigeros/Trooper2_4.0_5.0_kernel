...
//lcd
#if (defined(__M8510__)||defined(__M8522__))
int lcd_firmware_version[2] = {0,0};
int lcd_firmware_version_size = 2;
module_param_array(lcd_firmware_version, int, &lcd_firmware_version_size, 00444);
#endif
// ---------------------------------------------------------------------------
...
#if (defined(__M8510__)||defined(__M8522__))
    lcm_drv->compare_id();//add for read lcd lcd_firmware_version
#endif
...
static void _DISP_TargetLineCallback(void* pParam)
{
    ...
    if (0 == config_update_task_wakeup)
    {
        config_update_task_wakeup = 1;
        wake_up_interruptible(&config_update_wq);
...
unsigned int DISP_LCMTest(UINT32 fbVirAddr, UINT32 fbPhysAddr, UINT32 fb_size)
{
	...
	unsigned int *fb_buffer = (unsigned int*)fbVirAddr;
	if((LCM_TYPE_DSI == lcm_params->type && lcm_params->dsi.mode != CMD_MODE) || LCM_TYPE_DPI == lcm_params->type){//DBI
		return 1;
	}
...
//	memset((void*)fbVirAddr, color, fb_size*4);
	if(color == 0)
		color = 0xFF00FF00;
#if 1
	for(i=0;i<fb_size;i++)
...
#endif
/**///change OVL layer format
	mutex_lock(&OverlaySettingMutex);
	cached_layer_config[FB_LAYER].fmt = OVL_INPUT_FORMAT_PARGB8888;
	cached_layer_config[FB_LAYER].addr = fbPhysAddr;
	cached_layer_config[FB_LAYER].isDirty= 1;
