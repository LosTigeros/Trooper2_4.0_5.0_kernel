...
static int esd_recovery_kthread(void *data)
{
    ...
    for( ;; ) {
        ...
        #if (defined(OTM8018B_DSI_VDO)||defined(OTM8018B_WVGA_DSI_VDO))
         msleep(1000);       //2s
		#else
		msleep(2000);
		#endif
...
extern  unsigned int bak_BL_status;
bool backlight_is_off(void)
{
    printk("is_early_suspended = %d,bak_BL_status = %d\n",is_early_suspended,bak_BL_status);
  //  return (is_early_suspended || (bak_BL_status == 0));
   return (bak_BL_status == 0);  //pengjie  for hall
}

EXPORT_SYMBOL(backlight_is_off);
#endif
...
#if defined(__M8510__)  || defined(__M8515__) || defined(__M8512__)|| defined(__M8513__)//def LCD_CE_OFF_ENTER_CAMAPP        
//Added by fanjilin begin
extern LCM_DRIVER  *lcm_drv;
//Added by fanjilin end
static BOOL ColorEnhanceMutex = TRUE;
#endif
static int mtkfb_ioctl(struct file *file, struct fb_info *info, unsigned int cmd, unsigned long arg)
{
    ...
    DISP_STATUS ret = 0;
    int r = 0;
	/*konka-shenlili-for color mode-20130703-start*/
    unsigned char temp = 0xff;
	/*konka-shenlili-for color mode-20130703-end*/
#if 0
    MTKFB_FUNC();
#endif
...
//Added by fanjilin begin
	case 0x1128:
        if (lcm_drv->setColorMode == NULL) {
            return -EFAULT;
        }
        
        if (down_interruptible(&sem_early_suspend)) 
        {
            printk("[mtkfb_ioctl] down_interruptible can't get semaphore:%d\n", __LINE__);
        ...
        /*
        LCD_WaitForNotBusy();
        if (lcm_drv->setColorMode) 
        {
            lcm_drv->setColorMode(arg);
        }
        */
        if(ColorEnhanceMutex)
            DISP_PrepareSetColor(arg);

        up(&sem_early_suspend);	
        break;
        
    case 0x1129:	
        if (lcm_drv->getColorMode == NULL) {
            return -EFAULT;
        }
        ...
        LCD_WaitForNotBusy();
        //if (lcm_drv->getColorMode) 
        {
            if (lcm_drv->getColorMode(&temp) >= 0)
            {
                copy_to_user((void __user *)arg, &temp, sizeof(temp));
                up(&sem_early_suspend);
...
static int mtkfb_probe(struct device *dev)
{
    ...
    fbdev->fb_size_in_byte = MTK_FB_SIZEV;
    {
        ...
        if (mtkfb_enable_m4u)
        {
            ...
            ASSERT(fb_pa);
            printk("[MTKFB] FB MVA is 0x%08X PA is 0x%08X, mode:%d\n", fb_pa, (unsigned int)fbdev->fb_pa_base, DISP_IsDSIVdoMode());
        }
        else
        {
...
static void mtkfb_early_suspend(struct early_suspend *h)
{
#if defined(__M8510__) || defined(__M8515__) || defined(__M8512__)|| defined(__M8513__)
    ColorEnhanceMutex = FALSE;
#endif
//    struct mtkfb_device  *fbdev = (struct mtkfb_device *)mtkfb_fbi->par;
