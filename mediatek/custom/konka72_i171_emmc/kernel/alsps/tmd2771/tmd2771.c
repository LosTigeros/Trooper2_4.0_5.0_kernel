...
static int TMD2771_init_flag =0; // 0<==>OK -1 <==> fail
...
static int tmd2771_read_data_for_cali_call(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali);
...
static long tmd2771_enable_als(struct i2c_client *client, int enable)
{
	...
	EXIT_ERR:
		printk("tmd2771_enable_als fail\n");
		return res;
}
...
static long tmd2771_enable_ps(struct i2c_client *client, int enable)
{
	...
	err = tmd2771_read_data_for_cali_call(client,&ps_cali_temp);
	if(err)
	{
	   printk("%s():line=%d err!\n",__func__,__LINE__);
	   //goto exit;
...
EXIT_ERR:
	printk("tmd2771_check_and_clear_intr fail\n");
	return 1;
}
/*----------------------------------------------------------------------------*/
...
static int tmd2771_check_intr(struct i2c_client *client) 
{
	...
	//printk("tmd2771_check_and_clear_intr status=0x%x\n", buffer[0]);
	res = 1;
	intp = 0;
	intl = 0;
	if(0 != (buffer[0] & 0x20))
...
EXIT_ERR:
	printk("tmd2771_check_intr fail\n");
	return 1;
}
...
static int tmd2771_init_client_for_cali(struct i2c_client *client)
{
	...
	databuf[1] = TMD2771_CMM_PPCOUNT_VALUE;//0x02  (¹¦ÂÊ00~FF)
	res = i2c_master_send(client, databuf, 0x2);
	if(res <= 0)
	{
		goto EXIT_ERR;
...
static int tmd2771_get_ps_value(struct tmd2771_priv *obj, u16 ps)
{
	...
	//printk("tmd2771_get_ps_value  1 %d," ,ps_cali.close);
	//printk("tmd2771_get_ps_value  2 %d," ,ps_cali.far_away);
	//printk("tmd2771_get_ps_value  3 %d,", ps_cali.valid);

	//printk("tmd2771_get_ps_value  ps %d,", ps);
	...
	printk("333333:ps=%d,ps_cali.close=%,ps_cali.far_away= %d",ps,ps_cali.close,ps_cali.far_away);

	if(ps_cali.valid == 1)
		{
			//printk("tmd2771_get_ps_value val_temp  = %d",val_temp);
...
static void tmd2771_eint_work(struct work_struct *work)
{
	...
	{
		printk("tmd2771_eint_work check intrs: %d\n", err);
	}
	else
	{
		//get raw data
...
static tmd2771_WriteCalibration(struct PS_CALI_DATA_STRUCT *data_cali)
{
	   ...
	   printk("tmd2771_WriteCalibration  1 %d," ,data_cali->close);
		   printk("tmd2771_WriteCalibration  2 %d," ,data_cali->far_away);
		   printk("tmd2771_WriteCalibration  3 %d,", data_cali->valid);
...
static int tmd2771_read_data_for_cali(struct i2c_client *client, struct PS_CALI_DATA_STRUCT *ps_data_cali)
{
    ...
    for(i = 0;i<20;i++)
 	{
		...
		{
			printk("tmd2771_read_data_for_cali fail: %d\n", i); 
			break;
		}
		else
		{
    ...
    {
        data_cali = sum/20;
        printk("tmd2771_read_data_for_cali data = %d\n",data_cali);
        ...
        printk("tmd2771_read_data_for_cali close  = %d,far_away = %d,valid = %d\n",ps_data_cali->close,ps_data_cali->far_away,ps_data_cali->valid);
    }
    else
    {
...
/*		case ALSPS_SET_PS_CALI:
			...
			printk(" ALSPS_SET_PS_CALI %d,%d,%d\t",ps_cali_temp.close,ps_cali_temp.far_away,ps_cali_temp.valid);
			break;
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				tmd2771_read_ps(obj->client, &obj->ps);
                                ...
                                //mdelay(160);
				tmd2771_read_als_ch0(obj->client, &obj->als);
				printk("tmd2771_ps_operate als data=%d!\n",obj->als);
				sensor_data->values[0] = tmd2771_get_ps_value(obj, obj->ps);
				sensor_data->value_divide = 1;
				sensor_data->status = SENSOR_STATUS_ACCURACY_MEDIUM;			
			}
...
static int tmd2771_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	tmd2771_init_client_for_cali(client);
	err = tmd2771_read_data_for_cali(client,&ps_cali_temp);
	if(err)
	{
	   printk("%s():tmd2771_read_data_for_cali error!\n",__func__);
	   //goto exit;
	}
	...
	printk("tmd2771_init_client() OK!\n");

	if(err = misc_register(&tmd2771_device))
	{
		printk("tmd2771_device register failed\n");
...
/*	
	...
	{
		printk("tmd2771_delete_attr fail: %d\n", err);
	} 
*/
