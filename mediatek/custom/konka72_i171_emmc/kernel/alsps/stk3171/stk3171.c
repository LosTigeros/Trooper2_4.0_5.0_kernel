...
static int stk3171_local_init();
static int stk3171_remove();
...
{
	int i = 0,err = 0,j = 0;
	u16 data[12] = {0},sum = 0,data_cali = 0,offset = 0, maxval = 0;
	struct i2c_client *client;
	client = stk3171_i2c_client;
	struct stk3171_priv *obj = i2c_get_clientdata(client);
	printk("%s\n", __FUNCTION__);
	...
	mdelay(50);
	 ...
	 {
			mdelay(5);//50
			if(err = stk3171_read_ps(client,&data[i]))
			{
				APS_ERR("stk3171_read_ps fail: %d\n", i); 
				break;
			}
			mdelay(55);//160
	 }
	 printk("%s stk3171 10 ps data:\n", __FUNCTION__);
	...
	else
	{
	 ...
	 {
			data_cali = sum/10;
			printk("\n%s stk3171 read average ps data = %d\n", __FUNCTION__,data_cali);
	 }
	if(gain_setting >= (0x0d))
		offset = 15;
	else
		offset = 10;
	maxval -= data_cali;
	if(maxval >= offset)
		offset = maxval+5;
	// if(data_cali < (255-(offset*2)) )
	if(data_cali < atomic_read(&obj->ps_threshold_high))
	 {
		atomic_set(&obj->ps_threshold_high, data_cali + (offset*2) ); 
		atomic_set(&obj->ps_threshold_low, data_cali + offset ); 
		printk("%s Calibration PS High Threshold = %d, Low Threshold = %d\n", __FUNCTION__,atomic_read(&obj->ps_threshold_high),atomic_read(&obj->ps_threshold_low));
		if((err = stk3171_write_ps_high_thd(client, atomic_read(&obj->ps_threshold_high))))
		{
	 ...
	 {
		//printk("%s average ps data >= %d \nSet Default PS Threshold\n", (255-(offset*2)),__FUNCTION__);
		//atomic_set(&obj->ps_high_thd_val, data_cali - offset ); 
		//atomic_set(&obj->ps_low_thd_val, data_cali - (offset*2) ); 		
		printk("%s Calibration PS High Threshold = %d, Low Threshold = %d\n", __FUNCTION__,atomic_read(&obj->ps_threshold_high),atomic_read(&obj->ps_threshold_low));
		if((err = stk3171_write_ps_high_thd(client, atomic_read(&obj->ps_threshold_high))))
		{
			APS_ERR("write high thd error: %d\n", err);
...
static int stk3171_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (stk3171_init_client_for_cali() == 0)
		printk("%s stk3171_init_client_for_cali OK\n", __FUNCTION__);
	else
		printk("%s stk3171_init_client_for_cali Fail\n", __FUNCTION__);
	APS_LOG("%s: OK\n", __func__);
#if defined(MTK_AUTO_DETECT_ALSPS)	
    stk3171_init_flag = 0;
#endif	
...
#if defined(MTK_AUTO_DETECT_ALSPS)
    hwmsen_alsps_add(&stk3171_init_info);
#else
