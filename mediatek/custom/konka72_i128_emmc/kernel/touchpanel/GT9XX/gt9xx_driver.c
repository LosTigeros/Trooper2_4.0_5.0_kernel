...
static DEFINE_MUTEX(vps_sensor_mutex);

typedef struct str_ctp_ps 
{
    kal_bool bIsCtpPwrOn;
    kal_bool bIsPSPwrOn;

    kal_bool bIsPwrOn;
}STR_CTP_PS;

STR_CTP_PS strCtpPwr;
...
static void sTPDPowerOff(void)
{
    s32 ret = -1;
    
    mutex_lock(&vps_sensor_mutex);
    
    if ((0 != strCtpPwr.bIsCtpPwrOn) \
        || (0 != strCtpPwr.bIsPSPwrOn))
    {
        GTP_INFO("%s() can't power off (%d,%d)\n",__func__, \
            strCtpPwr.bIsCtpPwrOn, strCtpPwr.bIsPSPwrOn);

        mutex_unlock(&vps_sensor_mutex);
        return;
    }

    if (0 == strCtpPwr.bIsPwrOn)
    {
        GTP_INFO("%s() state same, return\n",__func__);

        mutex_unlock(&vps_sensor_mutex);
        return;
    }

    strCtpPwr.bIsPwrOn = 0;
...
static s32 i2c_set_tpd_ps_on(s32 enable)
{
    kal_uint32 uiTimeOut = 5;
    u8 state;
...
static s32 tpd_enable_ps(s32 enable)
{
    ...
    GTP_INFO("%s(%d, %d) pwr = %d !\n",__func__,enable, \
            strCtpPwr.bIsCtpPwrOn, strCtpPwr.bIsPwrOn);

    if (enable > 0)
    {
        strCtpPwr.bIsPSPwrOn = 1;
        sTPDPowerOn();
        
        msleep(50);
        
        ret = i2c_set_tpd_ps_on(enable);
        if (ret >= 0)
        {
            wake_lock(&ps_lock);
        }
        ...
        {
            GTP_ERROR("%s() ps pwer on fail!\n", __func__);

            strCtpPwr.bIsPSPwrOn = 0;
            sTPDPowerOff();
        }
    }
    else
...
s32 gtp_read_version(struct i2c_client *client, u16 *version)
{
    s32 ret = -1;
    s32 i;
    //Chenzz++ static ·ÀÖ¹Æô¶¯Ê±kernel stack Òì³£
    static u8 buf[8] = {GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff};
...
#ifdef TPD_PROXIMITY
static int gt9xx_alsps_probe(struct platform_device *pdev)
{
    GTP_DEBUG("%s enter!\n", __FUNCTION__);
    return 0;
}

static int gt9xx_alsps_remove(struct platform_device *pdev)
{
    GTP_DEBUG("%s enter!\n", __FUNCTION__);
    return 0;
}
...
static struct platform_driver gt9xx_alsps_driver = {   
    .probe      = gt9xx_alsps_probe,	
    .remove     = gt9xx_alsps_remove,
	.driver     = {
		.name  = "als_ps",
//		.owner = THIS_MODULE,
	}
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    tpd_firmware_version[0] = version_info >> 8;
    tpd_firmware_version[1] = version_info & 0xff;
...
static int touch_event_handler(void *unused)
{
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        if (!strCtpPwr.bIsCtpPwrOn)
        {//not send any ctp infor
            goto exit_work_func;
        }
    #endif
        ...
        if (touch_num > 1)
        {
            ...
            if (key_value || pre_key)
            {
                ...
                {
                    // input_report_key(tpd->dev, touch_key_array[i], key_value & (0x01 << i)) ;                if (key_value & (0x01 << i)) //key=1 menu ;key=2 home; key = 4 back;                    {                    input_x = 
                    if (key_value & (0x01 << i)) //key=1 menu ;key=2 home;
                    {
...
static int __init tpd_driver_init(void)
{
    ...
    GTP_INFO("MediaTek gt91xx touch panel driver init(%d)\n", TPD_I2C_NUMBER);

    i2c_register_board_info(TPD_I2C_NUMBER, &i2c_tpd, 1);
#else
    GTP_INFO("MediaTek gt91xx touch panel driver init() i2c 0\n");
