...
int kdCISModulePowerOn(CAMERA_DUAL_CAMERA_SENSOR_ENUM SensorIdx, char *currSensorName, BOOL On, char* mode_name)
{
    ...
    PK_DBG("kdCISModulePowerOn(%d): SensorIdx = %d\n",pinSetIdx, SensorIdx);
    ...
    if (On) {
       ...
       {
            PK_DBG("[CAMERA SENSOR]GC2035_YUV sensorIdx:%d \n",SensorIdx);
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name))
            {
                PK_DBG("[CAMERA SENSOR] Fail to enable digital power\n");
                //return -EIO;
	   ...
	   {
		   PK_DBG("[CAMERA SENSOR] kdCISModulePowerOn get in---BF3905_MIPI_YUV sensorIdx:%d; pinSetIdx=%d\n",SensorIdx, pinSetIdx);
       ...
       else // if (currSensorName && (0 == strcmp(SENSOR_DRVNAME_SP0A29_MIPI_YUV,currSensorName)))
       {
            PK_DBG("[CAMERA SENSOR]GC2035_YUV sensorIdx:%d \n",SensorIdx);
            if(TRUE != hwPowerOn(CAMERA_POWER_VCAM_D2, VOL_1800,mode_name))
            {
