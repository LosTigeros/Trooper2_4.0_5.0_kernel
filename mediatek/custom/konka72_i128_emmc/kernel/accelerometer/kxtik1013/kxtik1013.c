...
static struct i2c_board_info __initdata i2c_kxtik1013={ I2C_BOARD_INFO(KXTIK1013_DEV_NAME, (KXTIK1013_I2C_SLAVE_ADDR>>1))};
/*the adapter id will be available in customization*/
//static unsigned short kxtik1013_force[] = {0x00, KXTIK1013_I2C_SLAVE_ADDR, I2C_CLIENT_END, I2C_CLIENT_END};
//static const unsigned short *const kxtik1013_forces[] = { kxtik1013_force, NULL };
//static struct i2c_client_address_data kxtik1013_addr_data = { .forces = kxtik1013_forces,};
...
static int kxtik1013_init_flag =-1; // 0<==>OK -1 <==> fail
/*----------------------------------------------------------------------------*/
typedef enum {
    ADX_TRC_FILTER  = 0x01,
...
#define GSE_FUN(f)               printk("%s\n", __FUNCTION__)
#define GSE_ERR(fmt, args...)    printk("%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
#define GSE_LOG(fmt, args...)    printk(fmt, ##args)
...
static int KXTIK1013_CheckDeviceID(struct i2c_client *client)
{
	...
	if(false)
	{
		printk("KXTIK1013_CheckDeviceID 0x%x failt!\n ", databuf[0]);
		return KXTIK1013_ERR_IDENTIFICATION;
	}
	else
	{
		printk("KXTIK1013_CheckDeviceID 0x%x pass!\n ", databuf[0]);
	}
...
static int KXTIK1013_SetDataFormat(struct i2c_client *client, u8 dataformat)
{
	...
	printk("KXTIK1013_SetDataFormat OK! \n");
...
static int kxtik1013_init_client(struct i2c_client *client, int reset_cali)
{
	...
	printk("kxtik1013_init_client OK!\n");
#ifdef CONFIG_KXTIK1013_LOWPASS
...
/*----------------------------------------------------------------------------*/
/*
static int KXTIK1013_JudgeTestResult(struct i2c_client *client, s32 prv[KXTIK1013_AXES_NUM], s32 nxt[KXTIK1013_AXES_NUM])
{
    ...
    if ( test_result != 0xaa ) 
	{
        GSE_ERR("KXTIK1013_JudgeTestResult failt\n");
        res = -EINVAL;
    }
    return res;
}
...
/*----------------------------------------------------------------------------*/
int kxtik1013_operate(void* self, uint32_t command, void* buff_in, int size_in,
...
/*----------------------------------------------------------------------------*/
//static int kxtik1013_ioctl(struct inode *inode, struct file *file, unsigned int cmd,
//       unsigned long arg)
...
static int kxtik1013_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	memset(obj, 0, sizeof(struct kxtik1013_i2c_data));

	obj->hw = kxtik1013_get_cust_acc_hw();
	...
	sobj.self = obj;
    sobj.polling = 1;
    sobj.sensor_operate = kxtik1013_operate;
...
{
	struct acc_hw *hw = kxtik1013_get_cust_acc_hw();
	GSE_FUN();

	KXTIK1013_power(hw, 1);
	//kxtik1013_force[0] = hw->i2c_num;
