...
#if 1
    #define APS_FUN(f)               printk("%s\n", __FUNCTION__)
    #define APS_ERR(fmt, args...)    printk("%s %d : "fmt, __FUNCTION__, __LINE__, ##args)
...
#ifdef STK_TUNE0
//STK_MAX_MIN_DIFF > STK_HT_N_CT > STK_LT_N_CT
//挡跟不挡 差值的60% > STK_MAX_MIN_DIFF
...
static struct i2c_board_info __initdata i2c_stk3x1x={ I2C_BOARD_INFO("stk3x1x", (0x98>>1))};
#else
...
static int stk3x1x_local_init();
static int stk3x1x_remove();
int stk3x1x_init_flag = 0;
struct sensor_init_info stk3x1x_init_info = 
{       
    .name = "stk3x1x",      
...
/*----------------------------------------------------------------------------*/
int stk3x1x_get_addr(struct alsps_hw_stk3x1x *hw, struct stk3x1x_i2c_addr *addr)
{
	if(!hw || !addr)
	{
		return -EFAULT;
...
#endif
/*----------------------------------------------------------------------------*/
static void stk3x1x_power(struct alsps_hw_stk3x1x *hw, unsigned int on) 
{
...
//Chenzz++ 2014-1-2 防止同样环境光下,由于ALS RAW数据乱跳,导致背光闪烁
#ifdef __M8525__
{
    static unsigned long ulUpTick   = 0;
    static unsigned long ulDownTick = 0;
    static int iOldIdx = 0xff;

    //APS_DBG("Cur idx: %d, old idx = %d\n", idx, iOldIdx);
    if (iOldIdx == idx)
    {
        ulUpTick   = jiffies;
        ulDownTick = jiffies;
    ...
    else if (idx > iOldIdx)
    {
        ...
        else if (time_after(jiffies, ulUpTick + HZ))
        {
            iOldIdx  = idx;
            ulUpTick = jiffies;
            //APS_DBG("big timeout (%d, %d)\n", idx, iOldIdx);
        }
        else
        {
            idx = iOldIdx;
            //APS_DBG("big wait for timeout (%d, %d)\n", idx, iOldIdx);
        }
    }
    else
    {
        ...
        else if (time_after(jiffies, ulDownTick + HZ))
        {
            iOldIdx    = idx;
            ulDownTick = jiffies;
            //APS_DBG("low timeout (%d, %d)\n", idx, iOldIdx);
        }
        else
        {
            idx = iOldIdx;
            //APS_DBG("low wait for timeout (%d, %d)\n", idx, iOldIdx);
        }
    }
}
#endif
...
{
	struct alsps_hw_stk3x1x *hw = get_cust_alsps_hw_stk3x1x();
	struct stk3x1x_i2c_addr addr;
