...
#define KPD_PWRKEY_USE_HALL_EINT KPD_YES
 
#define KPD_PWRKEY_HALL_EINT         CUST_EINT_HALL_1_NUM
#define KPD_PWRKEY_HALL_DEBOUNCE     CUST_EINT_HALL_1_DEBOUNCE_CN
#define KPD_PWRKEY_HALL_POLARITY     CUST_EINT_HALL_1_POLARITY
#define KPD_PWRKEY_HALL_SENSITIVE    CUST_EINT_HALL_1_SENSITIVE
#define GPIO_HALL_EINT_PIN           GPIO_HALL_1_PIN
#define GPIO_HALL_EINT_PIN_MODE      GPIO_HALL_1_PIN_M_EINT
//CUST_EINT_HALL_1_DEBOUNCE_EN

#define KEY_FN_HALL  0x01e5

#endif


#if defined(__M8510__)
 
#define KPD_PWRKEY_USE_HALL_EINT KPD_YES
 
#if KPD_PWRKEY_USE_HALL_EINT
#define KPD_PWRKEY_HALL_EINT         2
#define KPD_PWRKEY_HALL_DEBOUNCE     0
#define KPD_PWRKEY_HALL_POLARITY     CUST_EINT_POLARITY_LOW
#define KPD_PWRKEY_HALL_SENSITIVE    CUST_EINT_EDGE_SENSITIVE
#define GPIO_HALL_EINT_PIN           GPIO136
#define GPIO_HALL_EINT_PIN_MODE      GPIO_MODE_04
#define KEY_FN_HALL  0x01e5
#endif
...
#define KPD_PWRKEY_USE_HALL_EINT KPD_NO
//KPD_YES 打开霍尔传感器
 
#if KPD_PWRKEY_USE_HALL_EINT
#define KPD_PWRKEY_HALL_EINT         15
#define KPD_PWRKEY_HALL_DEBOUNCE     0
#define KPD_PWRKEY_HALL_POLARITY     CUST_EINT_POLARITY_LOW
#define KPD_PWRKEY_HALL_SENSITIVE    CUST_EINT_EDGE_SENSITIVE
#define GPIO_HALL_EINT_PIN           GPIO101
#define GPIO_HALL_EINT_PIN_MODE      GPIO_MODE_02
#define KEY_FN_HALL  0x01e5
#endif
...
#if 0 // defined(__M8526__)
static void kpd_pwrkey_hall_eint_handler(unsigned long data)
{
    bool pressed;
    u8 old_state;

    pressed = 1;
    mt65xx_eint_mask(KPD_PWRKEY_HALL_EINT);

    mt_set_gpio_mode(GPIO_HALL_EINT_PIN, GPIO_MODE_00);
    mt_set_gpio_dir(GPIO_HALL_EINT_PIN, GPIO_DIR_IN);
    old_state = mt_get_gpio_in(GPIO_HALL_EINT_PIN);

    //if((backlight_is_off()) && (old_state == 1))
    if (old_state == 1)
    {
      //if(konkasmartclam==1||konkasmartclam==2)
    	{
          //konkasmartclamstate=3;            
    	    pressed = 1;
    	    input_report_key(kpd_input_dev, KPD_PWRKEY_MAP, pressed);
    	    input_sync(kpd_input_dev);       
    	    pressed = 0;
    ...
    //xj+input start
          pressed = 1;
    	    input_report_key(kpd_input_dev, KEY_FN_HALL, pressed); //report hall key to app
    	    input_sync(kpd_input_dev); 
          pressed = 0;
    	    input_report_key(kpd_input_dev, KEY_FN_HALL, pressed); //report hall key to app
    	    input_sync(kpd_input_dev); 
    //xj+input end	    
        ...
        printk("KPD_PWRKEY_USE_HALL_EINT:report Linux keycode = %u, LCD On\n", KEY_FN_HALL);
    }
	//else if(!backlight_is_off() && (old_state == 0))
	else if (old_state == 0)
	{
      //if(konkasmartclam==1||konkasmartclam==2)
    	{
            //konkasmartclamstate=2;                        
    	    pressed = 1;
    	    input_report_key(kpd_input_dev, KPD_PWRKEY_MAP, pressed);
    	    input_sync(kpd_input_dev); 	   
    	    pressed = 0;
        ...
        printk("KPD_PWRKEY_USE_HALL_EINT:report Linux keycode = %u, LCD Off\n", KPD_PWRKEY_MAP);
    }

    mt_set_gpio_mode(GPIO_HALL_EINT_PIN, GPIO_HALL_EINT_PIN_MODE);
    mt_set_gpio_dir(GPIO_HALL_EINT_PIN, GPIO_DIR_IN);

    /* for detecting the return to old_state */
    mt65xx_eint_set_sens(KPD_PWRKEY_HALL_EINT, KPD_PWRKEY_HALL_SENSITIVE);
    mt65xx_eint_set_polarity(KPD_PWRKEY_HALL_EINT, !old_state);
    mt65xx_eint_unmask(KPD_PWRKEY_HALL_EINT);

	kpd_print("KPD_PWRKEY_USE_HALL_EINT: GPIO142 Level= %d \n", old_state);
}

#endif

#if defined(__M8510__) || defined(__M8512__) || defined(__M8526__)
#if KPD_PWRKEY_USE_HALL_EINT
extern bool backlight_is_off(void);
static void kpd_pwrkey_hall_eint_handler(unsigned long data)
{
    bool pressed;
    u8 old_state;
	...
	kpd_print("KPD_PWRKEY_USE_HALL_EINT: GPIO142 Level= %d backlight_is_off=%d\n", old_state,backlight_is_off());
}
#endif
#endif
/*********************************************************************/
...
//#endif
...
#if defined(__M8510__) || defined(__M8526__) || defined(__M8512__)
#if KPD_PWRKEY_USE_HALL_EINT
	__set_bit(EV_KEY, kpd_input_dev->evbit);
    __set_bit(KPD_PWRKEY_MAP, kpd_input_dev->keybit);
	__set_bit(KEY_FN_HALL, kpd_input_dev->keybit);//xj+input	
#endif
#endif
	...
	mt65xx_eint_set_sens(KPD_PWRKEY_HALL_EINT, KPD_PWRKEY_HALL_SENSITIVE);
	mt65xx_eint_set_hw_debounce(KPD_PWRKEY_HALL_EINT, KPD_PWRKEY_HALL_DEBOUNCE);
	mt65xx_eint_registration(KPD_PWRKEY_HALL_EINT, true, KPD_PWRKEY_HALL_POLARITY,
	                         kpd_pwrkey_hall_eint_handler, false);
	mt65xx_eint_unmask(KPD_PWRKEY_HALL_EINT);

    printk("KPD_PWRKEY_USE_HALL_EINT: init %s(),HALL gpio mode = %d \n",__func__,mt_get_gpio_mode(GPIO_HALL_EINT_PIN));
#endif
#endif
