...
DSI_STATUS DSI_StartTransfer(bool isMutexLocked)
{
...
#if defined(ILI9806C_WVGA_DSI_VDO)|| defined(ILI9806E_WVGA_DSI_VDO)
#if defined(__M8513__)
		if (0 == mt_get_gpio_in(GPIO21)) //I158S兼容OTM8018B和ILI9806E屏
		{
#endif    
    while (DSI_REG->DSI_INTSTA.BUSY);
    OUTREG32(&DSI_REG->DSI_INTSTA, 0x0);
    ...
    OUTREGBIT(DSI_INT_ENABLE_REG, DSI_REG->DSI_INTEN, CMD_DONE, 0);
        ...
        OUTREG32(&DSI_CMDQ_REG->data[0], 0x00000100);
    OUTREG32(&DSI_REG->DSI_CMDQ_SIZE, 1);
...
#endif
#else
//Chenzz++ for __M8525__ FAQ08175: ILI9806C 开启ESD 会有概率性上方闪线
#ifdef ILI9806C_WVGA_DSI_VDO    
    while (DSI_REG->DSI_INTSTA.BUSY);
    OUTREG32(&DSI_REG->DSI_INTSTA, 0x0);
...
bool DISP_PrepareSetColor(unsigned int lmode)
{
#ifndef MT65XX_NEW_DISP
    bool result = false;
    ...
    //set DriverIC color enhance mode
    lcm_drv->setColorMode(lmode);
    //restore video mode
    //if(!result)
        OUTREG32(&DSI_REG->DSI_MODE_CTRL, AS_UINT32(&mode_ctl_backup));
